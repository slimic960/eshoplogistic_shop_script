        ...
        if(reload.length != 0) {
            switch (reload) {
                case 'offers':
                    ...
                    detail.offers = JSON.stringify(offers);
                    break
                case 'payment':
                    detail.payment = this.widget_payment;
                    break
                case 'city':
                    detail.city = this.widget_city
                    break
            }
            ...
            if(result.length > 0) {
                ...
                cityLinks.forEach.call(cityLinks, function(el){
                    el.addEventListener('click', event => {
                        ...
                        document.getElementById(_self.items.esldata_to_id).value = '{"fias":"'+el.dataset['fias']+'","services":'+el.dataset['services']+'}'
                        esl.run('city')
                    });
                })
            }
